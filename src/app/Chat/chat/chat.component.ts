...
export class ChatComponent implements OnChanges {
  ...
  constructor(
    ...
    this.chatService.chatFriendStatus.subscribe((statusData:any)=>{
      if (statusData) {
        this.roomList.forEach((room, index)=>{
          let friendData = statusData.find((i)=>i.cpp === room.chatWith.cpp);
          if (friendData) {
            this.roomList[index].accept = ((moment().unix() - friendData.last_active_time) > 60) ? false:true;
          }
        });
      }
    })
  ...
  setChatStatus( start=true ) {
    if (start == true) {
      let roomList = this.roomList.map((i)=>{
        return i.room;
      })
      this.chatService.emitCurrentStatus(roomList);
