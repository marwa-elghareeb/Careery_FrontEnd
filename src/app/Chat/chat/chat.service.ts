...
export class ChatService {
  ...
  private chatFriendStatusSource = new BehaviorSubject<string>(null);
  chatFriendStatus = this.chatFriendStatusSource.asObservable();
  ...
  constructor(private http: HttpClient) {
    ...
    this.socket.on('active-status', (statusData)=>{
      this.chatFriendStatusSource.next(statusData);
    });
  }

  emitCurrentStatus(room) {
    this.socket.emit('friend-active-status', room);
  }
