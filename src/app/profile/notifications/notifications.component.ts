...
export class NotificationsComponent implements OnInit {
  ...
  @ViewChild('connectModal', { static: true }) connectModal;
  chatwithUserCpp: any = '';
  ...
  constructor(
    ...
    this.chatService.getInvitation.subscribe((invitation: any) => {
      if (invitation) {
        ...
        let inviteRow:any = this.notifyConnectionList.findIndex((item) => {
      ...
      .subscribe((responseData: any) => {
        ...
        if (responseData.statustext == 'success') {
          ...
          this.notifyConnectionList = responseData.data.list.map((item) => {
            ...
            rowData['status'] = (((this.currentTime - item.last_active_time) > (1*60)))?false:true
