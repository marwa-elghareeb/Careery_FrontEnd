...
export class ProfileeditComponent implements OnInit {
  ...
  maxDayFrom: any = new Date(); // 
  maxDayTo:any = new Date();
  ...
  bodYear: Array<any> = []; // birthday years
  ...
  submitExperienceFrom() {
    ...
    if (!this.experienceForm.company_name || !this.experienceForm.position_id || !this.experienceForm.from_year) {
      ...
      let duplicateExperience:any = this.userExperiencelist.filter((item)=>{
        return (
      ...
      if (duplicateExperience && duplicateExperience.length > 0 && duplicateExperience[0].id != this.experienceForm.id) {
  ...
  onFileChanged(event) {
    ...
    if (uploadImage && uploadImage.type.indexOf('image/') == -1 && uploadImage.type.indexOf('/pdf') == -1) {
    ...
    if (uploadImage && uploadImage.size > 500000) {
      ...
      this.certificatePicture = uploadImage.name;
...
*/
  ...
  */
  resetProjectForm() {
    ...
    this.maxDayTo = new Date();
    this.maxDayFrom = new Date();
  }
...
*/
  ...
  selectProjectDate($event) {
    if ($event) {
      this.projectForm.to = null;
      let fromDate = new Date($event);
      this.maxDayTo = fromDate;
    }
  }
}
